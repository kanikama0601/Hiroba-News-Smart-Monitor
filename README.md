...
`helm` チャートを作成しているため、`k8s` クラスタ上にデプロイすることができます。  
動作検証は `k3s` を用いて行っていますが、大きな差異はないと考えます。  

デフォルトでは `LoadBalancer` で `8888` ポートに公開されます。  
`Ingress` 等を使いたい場合は `values.yaml` で設定してください。
